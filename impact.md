...
결과
상품 등록 프로세스를 일반 등록 → 간편 등록 → 대량 등록 구조로 발전시켜 반복 업무를 자동화했다.
운영자가 개별 상품마다 입력하던 작업을 배치 단위로 처리할 수 있게 됐다.
옵션 상품, 쇼핑몰별 설정, 재시도 및 데드락 대응을 포함한 안정적인 등록 프로세스를 구축했다.

3. 
제목
Rule-based 전략과 Google ADK 기반 투자 에이전트를 결합하여 종목 분석, 매수 · 매도 판단, 자동 매매 실행이 가능한 멀티 에이전트 구조 구현
...
결과
자동 에이전트가 실제 주문장에 매수/매도 주문을 공급하면서 미체결 주문과 체결 이벤트가 자연스럽게 발생할 수 있게 됐다.
체결 시 주가, 캔들, 거래량, 포트폴리오, 거래 내역, 실시간 업데이트가 함께 갱신되어 시장 화면이 더 살아 움직이게 됐다.
판단 로그와 주문 ID가 연결되어 ADK 판단, rule-based fallback, 거절/실패 사유, 실행 결과를 추적할 수 있는 구조가 생겼다.